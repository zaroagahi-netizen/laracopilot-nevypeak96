...
<div class="md:hidden" x-data="{ open: false }" @click.away="open = false">
    <button @click="open = !open" class="flex items-center gap-2 px-3 py-2 rounded-lg bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700">
        ...
        <span class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ \App\Helpers\LocaleHelper::getCurrentLocaleName() }}</span>
    </button>
        ...
        <div class="py-2">
            @foreach(config('laravellocalization.supportedLocales') as $code => $locale)
                <a href="{{ url($code . '/' . ltrim(request()->path() === '/' ? '' : preg_replace('/^[a-z]{2}(-[a-z]{4})?\//i', '', request()->path()), '/')) }}" 
                   class="flex items-center gap-3 px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700 {{ app()->getLocale() === $code ? 'bg-blue-50 dark:bg-blue-900/20' : '' }}"
